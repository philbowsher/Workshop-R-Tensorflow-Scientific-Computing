# ============================================================================
# Deploy Plumber API to Posit Connect (Programmatic Deployment)
# ============================================================================
#
...
cat("Deploying Plumber API to Posit Connect...\n")
...
# Deploy the API
cat("Deploying Plumber API...\n")
setwd("~/Workshop-R-Tensorflow-Scientific-Computing/part_3/immunotherapy/3_share_model_plumber")
...
deployment <- rsconnect::deployApp(
  appDir = ".",
  appTitle = "Peptide Prediction API",
  appFiles = c("plumber.R", "requirements.txt"),
  server = server_name,
...
if (is.character(deployment)) {
  ...
  if (nrow(deployments) > 0) {
    ...
    api_url <- "https://pub.workshop.posit.team/plumber/"
  }
}
...
cat("  Sys.setenv(CONNECT_CONTENT_URL = 'plumber')\n")
